...
function te = temppred_avg(lon,lat,depth,mixlay,t,ut,vt,temp,long,lati,mini,maxi,S,Tdep,nrsamp,day,avg)
...
%% Defines whether average over time is taken or not for current movement
if avg == 1
xt = mean(xt(:,optrap:cltrap,:,:),2,'omitnan');
yt = mean(yt(:,optrap:cltrap,:,:),2,'omitnan');
end
...
%% Defines whether average over time is taken or not for temperature and mixlayer
if avg == 1
    v = 1;
else
    v = optrap : cltrap;
...
if avg == 1
temptot = mean(temptot(:,v,:,:),2,'omitnan');
%temptotstd = std(temptot(:,v,:,:),2,'omitnan');
mixlay = mean(mixlay(:,:,v),3,'omitnan');
else
    temptot=temptot;
    mixlay=mixlay;
end

%% Finding location of origin
...
mix_dep_temp_std = mean(mix_dep_tempts,3,'omitnan');
...
max_prod_temp_std = mean(max_prod_tempts,3,'omitnan');
...
phot_zo_temp_std = mean(phot_zo_tempts,3,'omitnan');
...
sst_temp_std = mean(sst_tempts,3,'omitnan');
...
%nrsamp = 18;
depthD47 = zeros(length(depth),length(v),length(lon),length(lat));
...
for i = 1 : 3
    for ii = 1 : 3
        for iii = ii + 3
            for iv = i + 3
                for vix = iii + 3
               D47_mix(i,ii) = mix_dep_tempt(i,ii,v(1));
               D47_max(i,ii) = max_prod_tempt(i,ii,v(1));
               D47_phot(i,ii) = phot_zo_tempt(i,ii,v(1));
               D47_sst(i,ii) = sst_tempt(i,ii,v(1));
               ...
               D47_mix(i,vix) = mix_dep_tempt(i,ii,v(end));
               D47_max(i,vix) = max_prod_tempt(i,ii,v(end));
               D47_phot(i,vix) = phot_zo_tempt(i,ii,v(end));
               D47_sst(i,vix) = sst_tempt(i,ii,v(end));
               D47_mix(iv,ii) = mix_dep_tempts(i,ii,v(1));
               D47_max(iv,ii) = max_prod_tempts(i,ii,v(1));
               D47_phot(iv,ii) = phot_zo_tempts(i,ii,v(1));
               D47_sst(iv,ii) = sst_tempts(i,ii,v(1));
               ...
               D47_mix(iv,vix) = mix_dep_tempts(i,ii,v(end));
               D47_max(iv,vix) = max_prod_tempts(i,ii,v(end));
               D47_phot(iv,vix) = phot_zo_tempts(i,ii,v(end));
               D47_sst(iv,vix) = sst_tempts(i,ii,v(end));
                end
            end
        end
    end
...
if ti == 1
    temptot1 = mean(temptot(depthmin:depthmax,ti,:,:),'omitnan');
else
    temptot1 = mean(mean(temptot(depthmin:depthmax,ti,:,:),'omitnan'),'omitnan');
end
for i = 1:length(lon)
    for ii = 1:length(lat)
